...
/*
* Quasi-random number generator based on:
http://extremelearning.com.au/unreasonable-effectiveness-of-quasirandom-sequences/
* Generates points in a unit hypercube in Dim dimensions.
*
* The dimension of the generator can be chosen at runtime if the Dim argument is
set to DYNAMIC.

*/
template <std::size_t Dim, typename RealType = double> class QuasiRandom {
public:
  ...
  /* Construct a generator using the specified seed. The seed must be in the
   * range [0.0, 1.0). */
  ...
  /* Restart the sequence using a new seed. The seed must be in the range
   * [0.0, 1.0). */
...
private:
  RealType seed_;
  state_type alpha_; /* The initial point of the sequence (without considering
                        the seed). */
  state_type point_; /* The previous point of the sequence. */
};

/* Can't generate points in 0 dimensions. */
template <typename RealType> class QuasiRandom<0, RealType>;

/* DYNAMIC GENERATOR */

/* Specialization of the generator with a dynamic number of dimensions. */
template <typename RealType> class QuasiRandom<DYNAMIC, RealType> {
public:
  using result_type = std::vector<RealType>;
  using state_type = std::vector<RealType>;
  using size_type = std::size_t;

  /* Construct a generator in dim dimensions. The seed must be in the range
   * [0.0, 1.0). */
...
QuasiRandom<Dim, RealType>::QuasiRandom() noexcept : seed_(0.5) {
...
template <std::size_t Dim, typename RealType>
QuasiRandom<Dim, RealType>::QuasiRandom(RealType seed) : seed_(seed) {
  if (!(0.0 <= seed && seed < 1.0))
    throw std::invalid_argument("The seed must be in the range [0.0, 1.0).");
...
template <std::size_t Dim, typename RealType>
constexpr auto QuasiRandom<Dim, RealType>::operator()() noexcept
    -> result_type {
...
QuasiRandom<DYNAMIC, RealType>::QuasiRandom(size_type dim, RealType seed)
  ...
  if (dim == 0)
    throw std::invalid_argument(
        "The dimension of the generator must be at least 1.");
  if (!(0.0 <= seed && seed < 1.0))
    throw std::invalid_argument("The seed must be in the range [0.0, 1.0).");
...
template <typename RealType>
constexpr auto QuasiRandom<DYNAMIC, RealType>::dim() const noexcept
    -> size_type {
  return dim_;
}
...
RealType phi(std::size_t dim, std::size_t n) noexcept {
  ...
  while (n--)
    phid = std::pow(1.0 + phid, exponent);
