...
TEST_CASE("dynamic generator") {
  ...
  SECTION("types") {
    STATIC_REQUIRE(
        std::is_same_v<decltype(qrng)::result_type, std::vector<double>>);
    STATIC_REQUIRE(
        std::is_same_v<decltype(qrng)::state_type, std::vector<double>>);

    QuasiRandom<DYNAMIC, float> fqrng(2);

    STATIC_REQUIRE(
        std::is_same_v<decltype(fqrng)::result_type, std::vector<float>>);
    STATIC_REQUIRE(
        std::is_same_v<decltype(fqrng)::state_type, std::vector<float>>);
  }
}
...
TEST_CASE("static size") {
  ...
  SECTION("types") {
    STATIC_REQUIRE(std::is_same_v<decltype(qrng)::result_type,
                                  std::array<double, qrng.dim()>>);
    STATIC_REQUIRE(std::is_same_v<decltype(qrng)::state_type,
                                  std::array<double, qrng.dim()>>);

    QuasiRandom<5, float> fqrng;

    STATIC_REQUIRE(
        std::is_same_v<decltype(fqrng)::result_type, std::array<float, 5>>);
    STATIC_REQUIRE(
        std::is_same_v<decltype(fqrng)::state_type, std::array<float, 5>>);
  }
}
